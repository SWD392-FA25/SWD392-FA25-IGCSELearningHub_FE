...
export function AssignmentSubmitModal({
  ...
  const handleSubmit = async () => {
    ...
    if (!textAnswer.trim() && !attachmentLink.trim()) {
      toast.error("Please provide either a text answer or an attachment link");
      return;
    }
    ...
    try {
      const response = await assignmentService.submitAssignment(assignment.assignmentId, {
        attachmentUrl: attachmentLink.trim(),
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Enter link to your file (e.g., Google Drive, Dropbox, etc.)"
              disabled={submitting}
              type="url"
            />
            <p className="text-xs text-muted-foreground mt-1">
              Provide a public link to your file if needed.
            </p>
          </div>
        </div>
      )}
